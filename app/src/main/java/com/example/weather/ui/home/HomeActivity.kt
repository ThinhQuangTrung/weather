...
import com.example.weather.ui.favourite.FavouriteFragment
 ...
 * - Điều hướng mượt mà giữa các Fragment: HomeFragment, ForecastFragment, FavouriteFragment.
 * - Khôi phục và quản lý trạng thái Fragment chính xác sau khi Activity recreate.
 */
@AndroidEntryPoint
class HomeActivity : BaseActivity() {
    ...
    private fun setupFragments(savedInstanceState: Bundle?) {
        if (savedInstanceState == null) {
            ...
            val favourite = FavouriteFragment.newInstance()
            ...
            supportFragmentManager.beginTransaction()
                .add(R.id.fragmentContainer, forecast, TAG_FORECAST).hide(forecast)
                .add(R.id.fragmentContainer, favourite, TAG_FAVOURITE).hide(favourite)
            ...
            homeFragment = supportFragmentManager.findFragmentByTag(TAG_HOME) as? HomeFragment
                ?: HomeFragment.newInstance().also {
                    supportFragmentManager.beginTransaction()
                        .add(R.id.fragmentContainer, it, TAG_HOME).commit()
                }
            ...
            forecastFragment = supportFragmentManager.findFragmentByTag(TAG_FORECAST) as? ForecastFragment
                ?: ForecastFragment.newInstance().also {
                    supportFragmentManager.beginTransaction()
                        .add(R.id.fragmentContainer, it, TAG_FORECAST).hide(it).commit()
                }

            favouriteFragment = supportFragmentManager.findFragmentByTag(TAG_FAVOURITE) as? FavouriteFragment
                ?: FavouriteFragment.newInstance().also {
                    supportFragmentManager.beginTransaction()
                        .add(R.id.fragmentContainer, it, TAG_FAVOURITE).hide(it).commit()
                }
    ...
    private fun setupBottomNavigation() {
        binding.tabHome.setOnClickListener {
            val target = homeFragment
                ?: (supportFragmentManager.findFragmentByTag(TAG_HOME) as? HomeFragment)
            if (target != null) switchFragment(target, 0)
        }

        binding.tabForecast.setOnClickListener {
            val target = forecastFragment
                ?: (supportFragmentManager.findFragmentByTag(TAG_FORECAST) as? ForecastFragment)
            if (target != null) switchFragment(target, 1)
        }

        binding.tapfavourite.setOnClickListener {
            val target = favouriteFragment
                ?: (supportFragmentManager.findFragmentByTag(TAG_FAVOURITE) as? FavouriteFragment)
            if (target != null) switchFragment(target, 2)
        }
    }
    ...
    private fun switchFragment(targetFragment: Fragment, tabIndex: Int) {
        ...
        when (targetFragment) {
            is FavouriteFragment -> targetFragment.reload()
            is HomeFragment -> targetFragment.refreshFavoriteState()
        }
    }
    ...
    fun navigateToHome(cityIndex: Int = -1) {
        ...
        if (target != null) {
            ...
            if (cityIndex >= 0) target.selectCityIndex(cityIndex)
        }
    }
    ...
    private fun updateTabUI(selectedIndex: Int) {
        ...
        binding.pillHome.setBackgroundResource(
            if (isHome) R.drawable.bg_nav_active_pill else android.R.color.transparent)
        ...
        binding.pillForecast.setBackgroundResource(
            if (isForecast) R.drawable.bg_nav_active_pill else android.R.color.transparent)
        ...
        binding.pillFavourite.setBackgroundResource(
            if (isFavourite) R.drawable.bg_nav_active_pill else android.R.color.transparent)
